...
socket.on("room-created", (id, username) => {
    playerId = 1;
    roomId = id;
    username = username

    setPlayerTag(1,username);

    startScreen.style.display = "none";
    gameplayScreen.style.display = "block";
})

socket.on("room-joined", (id, username) => {
    playerId = 2;
    roomId = id;

    playerOneConnected = true;
    playerJoinTheGame(1)
    setPlayerTag(2,username);
...
// Functions
function setPlayerTag(playerId,username){
    if(playerId === 1){
        playerOneTag.innerText = `You (${username})`;
        playerTwoTag.innerText = "Enemy (Player 2)";
    }else{
        playerOneTag.innerText = "Enemy (Player 2)";
        playerTwoTag.innerText = `You (${username})`;
    }
}
